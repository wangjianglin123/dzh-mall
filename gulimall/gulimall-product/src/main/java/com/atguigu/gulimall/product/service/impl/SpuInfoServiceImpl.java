...
public class SpuInfoServiceImpl extends ServiceImpl<SpuInfoDao, SpuInfoEntity> implements SpuInfoService {
    ...
    @Autowired
    CouponFeignService couponFeignService;  //调用远程的数据
    ...
    public void saveSpuInfo(SpuSaveVo vo) {
        ...
        //4.保存spu的规格参数 pms_product_attr_value : map.stream()是遍历list的对象
        ...
        R r = couponFeignService.saveSpuBounds(spuBoundTo);//这里涉及跨域问题，就用openfeign
        if (r.getCode() != 0) {
    ...
    public void up(Long spuId) {
        ...
        //1.查出当时spuid对应得所有得sku信息，品牌名字
        List<SkuInfoEntity> skus = skuInfoService.getSkusBySpuId(spuId);
        // 批次得到skuId
        List<Long> skuIdList = skus.stream().map(SkuInfoEntity::getSkuId).collect(Collectors.toList());


        //4.查询当前sku的所有可以被检索的所有规格属性
        List<ProductAttrValueEntity> baseAttrs = productAttrValueService.baseAttrlistforspu(spuId);
        List<Long> attrIds = baseAttrs.stream().map(attr -> {
            return attr.getAttrId();
        }).collect(Collectors.toList());
        //在指定的所有属性集合里面，挑选出所有的检索属性
        ...
        if (r.getCode() == 0) {
            ...
            /**
             * 1.构造请求数据，将对象转为jsoon
             * 2。发送请求进行执行(执行陈工辉解码响应数据)
             * 3. 执行请求会有重试机制
             *     while(true){
             *         exectuteAndDecode(template);
             *     }catch(){
